...
class MSRVTTDataset(Dataset):
    ...
    def __init__(self, config: Config, split_type='train', img_transforms=None):
        ...
        trainset_with_titles = 'data/MSRVTT/train_caption_per_video.json'
        testset_with_titles = 'data/MSRVTT/test_caption_per_video.json'
        ...
        self.train_titles = load_json(trainset_with_titles)

        self.test_tiles = load_json(testset_with_titles)
    ...
    def __getitem__(self, index):
        video_path, caption, titles, video_id = self._get_vidpath_and_caption_by_index(index)
    ...
    def _get_vidpath_and_caption_by_index(self, index):
        ...
        if self.split_type == 'train':
            ...
            titles = self.train_titles[vid]
        else:
            vid = self.test_df.iloc[index].video_id
            video_path = os.path.join(self.videos_dir, vid + '.mp4')
            caption = self.test_df.iloc[index].sentence
            titles = self.test_tiles[vid]

        return video_path, caption, titles, vid
